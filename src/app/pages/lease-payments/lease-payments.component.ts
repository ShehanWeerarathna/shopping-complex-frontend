...
export class LeasePaymentsComponent implements OnInit{
  ...
  constructor(private route: ActivatedRoute,private leasePaymentService:LeasePaymentService) { }
  ...
  refreshLeasePayments(leaseAgreementId:number){
    this.leasePaymentService.getLeasePaymentsAsync(this.currentPage,this.pageSize,leaseAgreementId).subscribe(
      leasePayments => {
